...
taus <- c()
for (i in 1:1000) {
  random.swap <- sample(c(TRUE, FALSE), dim(orig.df)[1], replace=TRUE)
  random.swap = as.numeric(random.swap)
  orig.df$Z = random.swap
  
  tau <- coef(lm(RET_FT4_PCT_CHANGE ~ ., data=orig.df[,!(names(orig.df) %in% irr.cols)]))["Z"]
  taus <- append(taus, tau)
}
